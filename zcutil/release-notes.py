...
def generate_release_note(version, prev, clear):
    filename = 'release-notes.md'.format(version)
    ...
    if prev:
        ...
        latest_tag = subprocess.Popen(['git describe --always'], shell=True, stdout=subprocess.PIPE, text=True).communicate()[0].replace('\n', ' ')
    print("Previous release tag: ", latest_tag)
    notes = subprocess.Popen(['git shortlog {0}...origin/dev'.format(version)], shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE).communicate()[0]
