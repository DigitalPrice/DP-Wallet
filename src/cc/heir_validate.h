...
inline static bool isMyFuncId(uint8_t funcid) {
    return IS_CHARINSTR(funcid, "FAC");
}
inline static bool isSpendingTx(uint8_t funcid) {
    return (funcid == 'C');
}
...
class CNormalInputIdentifier : CInputIdentifierBase  {
  public:
    ...
    virtual std::string inputName() const {
        return std::string("normal input");
    }
...
template <class Helper> class CCC1of2AddressValidator : CValidatorBase {
  public:
    ...
    virtual bool isVinValidator() const {
        return false;
    }
...
template <class Helper> class CMarkerValidator : CValidatorBase {
  public:
    ...
    virtual bool isVinValidator() const {
        return true;    // this is vin validator
    }
    virtual bool validateVout(CTxOut vout, int32_t vout_n, std::string& message) const {
        return true;
    }
