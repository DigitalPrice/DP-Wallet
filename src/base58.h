...
class CZCSproutPaymentAddress : public CZCEncoding<libzcash::SproutPaymentAddress, CChainParams::ZCPAYMENT_ADDRRESS, libzcash::SerializedSproutPaymentAddressSize> {
  protected:
    std::string PrependName(const std::string& s) const {
        return "payment address" + s;
    }
  public:
    CZCSproutPaymentAddress() {}

    CZCSproutPaymentAddress(const std::string& strAddress) {
        SetString(strAddress.c_str(), 2);
    }
    CZCSproutPaymentAddress(const libzcash::SproutPaymentAddress& addr) {
        Set(addr);
    }
};
